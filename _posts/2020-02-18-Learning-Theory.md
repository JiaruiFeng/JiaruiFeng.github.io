...
Let $ \mathcal{B}_i $ denote the bad event of $ h_i $, that is $ E_{\mathrm{in}}(h_i)-E_{\mathrm{out}}(h_i)>\epsilon $ , We have:
...
In many practical applications, the data we learn from are not generated by a determinsitc target function. Instead, they are generated in a noisy way such that the output is not uniquely determind by the input. This situation can be radily modeled within the same framework. Instead of $y=f(x)$, 

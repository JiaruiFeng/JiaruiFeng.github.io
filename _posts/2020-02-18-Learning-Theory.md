...
In many practical applications, the data we learn from are not generated by a determinsitc target function. Instead, they are generated in a noisy way such that the output is not uniquely determind by the input. This situation can be radily modeled within the same framework. Instead of $y=f(x)$, we can regard $y$ as a random variable that is affected by input $x$.
